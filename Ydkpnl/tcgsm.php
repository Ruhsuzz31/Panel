...
<body>
    <div class="container">
        ...
        if (isset($_GET['tc'])) {
            ...
            $url = "https://apiv2.tsgonline.net/tsgapis/OrramaKonmaBurragaKoy/tcgsm.php?auth=tsgxyunus&tc=" . urlencode($tc);
                ...
                if (json_last_error() !== JSON_ERROR_NONE) {
                    ...
                    if (isset($data['success']) && $data['success'] === true) {
                        if (isset($data['data']) && is_array($data['data']) && count($data['data']) > 0) {
                            ...
                            echo "<tr><th>TC Kimlik No</th><th>GSM Numarası</th></tr>";
                            
                            foreach ($data['data'] as $item) {
                                $tc_no = htmlspecialchars($item['TC'] ?? 'Bilinmiyor');
                                $gsm = htmlspecialchars($item['GSM'] ?? 'Bilinmiyor');
                                echo "<tr><td>{$tc_no}</td><td>{$gsm}</td></tr>";
                            }
